...
class Trial:
    def __init__(
        self, group: str, subject: float, age: int, education: int, trial: str
    ) -> None:
    ...
    def set_recognition(
        self, data, oldImage: str, newImage: str, isCorrect: bool, isOldRight: bool
    ):
    ...
    def build_scanpath(self, line_width=5, point_size=7, isEncoding=True, resize=None):
        ...
        if isEncoding:
            ...
            coords = list(
                zip(self.recognition[:, 1] - 100, self.recognition[:, 2] - 170)
            )
        ...
        draw.line(xy=coords, fill=0, width=line_width)

        for coord in coords:
            draw.regular_polygon(
                bounding_circle=(coord, point_size),
                n_sides=20,
                fill=0,
                width=point_size,
            )
    ...
    def build_heatmap(self, distance, angle, isEncoding=True, resize=None):
        ...
        if isEncoding:
            ...
            coords = list(
                zip(self.recognition[:, 1] - 800, self.recognition[:, 2] - 450)
            )
...
class VTNet_Dataset(Dataset):
    ...
    def __init__(
        ...
        if len(scanpaths) == 0:
            ...
            for file in [x for x in os.listdir(path) if x[-3:] == "pkl"]:
                ...
                with open(path + file, "rb+") as f:
                    self.subject += list(
                        map(
                            lambda x: x.subject if x.encoding is not None else None,
                            pickle.load(f),
                        )
                    )
                with open(path + file, "rb+") as f:
                    self.image += list(
                        map(
                            lambda x: x.oldImage if x.encoding is not None else None,
                            pickle.load(f),
                        )
                    )
                with open(path + file, "rb+") as f:
                    self.score += list(
                        map(
                            lambda x: x.isCorrect if x.encoding is not None else None,
                            pickle.load(f),
                        )
                    )
            ...
            self.rawdata = np.array(
                [
                    (
                        np.pad(rd, ((max_len - len(rd), 0), (0, 0)), constant_values=0)
                        if hasattr(rd, "__iter__")
                        else None
                    )
            ...
            self.rawdata = torch.tensor(
                np.array(list(self.rawdata)), dtype=torch.float32
            )
            ...
            self.subject = torch.tensor(
                np.array(self.subject)[self.groups is not None].astype(np.float16)
            )
            self.scanpaths = np.array(self.scanpaths, dtype=object)[
                self.groups is not None
            ]

            self.scanpaths = (
                [pil_to_tensor(rd) for rd in self.scanpaths]
                if not useHeatmap
                else np.array(list(self.scanpaths), dtype=np.uint8)
            )
            self.scanpaths = (
                torch.stack(self.scanpaths, dim=0)
                if not useHeatmap
                else torch.unsqueeze(
                    torch.tensor(self.scanpaths, dtype=torch.uint8), dim=1
                )
            )

            self.score = torch.tensor(
                np.array(self.score)[self.groups is not None].astype(bool)
            )
            self.image = np.array(self.image)[self.groups is not None].astype(str)
            self.groups = torch.tensor(self.groups[self.groups is not None].astype(int))
        ...
        if IMAGE_DIR != "":
            ...
            for im in os.listdir(f"{IMAGE_DIR}"):
                images[im] = (
                    torch.unsqueeze(
                        read_image(IMAGE_DIR + im, mode=ImageReadMode.GRAY), dim=0
                    )
                    if imageMode == "BW"
                    else torch.unsqueeze(
                        read_image(IMAGE_DIR + im, mode=ImageReadMode.RGB), dim=0
                    )
                )
            output_image = []
            ...
            self.scanpaths = (
                self.scanpaths * output_image
                if imageMode == "MIX"
                else torch.cat((self.scanpaths, output_image), dim=1)
            )
            del output_image
    ...
    def __getitem__(self, index):
        if self.score != []:
            return (
                [self.rawdata[index, :, 1:], self.scanpaths[index], self.score[index]],
                self.groups[index],
            )
        else:
            return [self.rawdata[index], self.scanpaths[index]], self.groups[
                index
            ]
